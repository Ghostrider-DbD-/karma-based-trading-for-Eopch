...
if !(isNull EPOCH_lastNPCtradeTarget) then {
	...
	if (isClass (_config >> _uiItem)) then{
		...
		if (_ItemIndex > -1) then {
			...
			/* New Code to handle Karma-based Trading added 12/10/21 */		
			/* This script handles checks for items in the trader's inventory so karma checks are needed */	
			if (_allowAdd) then {
				private _karmaChecks = [_uiItem] call GRG_fnc_canBuyItem;
				//diag_log format["npcTraderAdd2: _karmachecks = %1",_karmaChecks];
				_karmaChecks params ["_canBuy","_msg"];
				if !(_canBuy) then {
					_allowAdd = false;
					_errormsg = _msg;
				};
